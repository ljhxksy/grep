...
/*
* insert()
* Purpose: Insert Value and Entry into the hash map
* Parameters: a reference to the key string, a reference to the Location struct
* Return: none
* Effect: Value and Entry added into the hash map
*/
...
* Return: Returns the populated Value.
*/
Value HashMap::insertValue(string &key, Location &fileInfo) {
    Value val;
    val.word = toLowerCase(key);
    Entry e = insertEntry(key, fileInfo);
...
*          vector.     
* Parameters: Reference to the word and the Location of that word.
* Return: Returns the populated Entry.
* Effect: This function increments size.
*/
...
void HashMap::expand() {
    ...
    for (size_t i = 0; i < capacity; i++) {
        new_table.push_back(vector<Value>());
    }
...
bool HashMap::is_loaded() {
    return ((float)size/(float)capacity > load_factor);
}
...
void HashMap::search(string &word, ostream &output, bool isSensitive) {
    ...
    int idx = getHashIndex(stripNonAlphaNum(toLowerCase(word)));
    Value val;
    ...
    if (isSensitive) {
        ...
        queryInsensitive(val, word, output);
    }
}
...
void HashMap::queryInsensitive(Value &val, string &word, ostream &output) {
    ...
    for (auto &vars : val.variation) {
        for (auto &infos : vars.info) {
            string hehe = dir[infos.fileIndex].lines[infos.lineIndex];
            string lowerLine = 
                   toLowerCase(dir[infos.fileIndex].lines[infos.lineIndex]);
            if (lowercasedWords.find(lowerLine) == lowercasedWords.end()) {
                ...
                lowercasedWords.insert(lowerLine);  
            }
        }
    }
